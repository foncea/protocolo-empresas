...
fraccion_tiempo_sano = function(sim){
    ...
    resultados$X = NULL
    out = resultados %>%
        filter(estado == 'infeccioso' & actividad == 'trabajo' & cantidad > 0) %>%
        group_by(it) %>%
        summarize(inf = n() / 92) %>%
        ungroup() %>%
        summarize(fraccion_tiempo = 1 - mean(inf)) %>%
        mutate(Protocolo = nombre[n])
    return(out)
}
...
{
    n=1
    fraccion_tiempo = fraccion_tiempo_sano(path_sim2(n))
    
    for(n in 2:length(alg)){
        aux =  fraccion_tiempo_sano(path_sim2(n))
...
# Poblacion infectada trabajando
{
    ...
    #    mutate(Protocolo = factor(Protocolo, levels=nombre[rev(c(2,3, 1))])) %>%
        ggplot(aes(x=tiempo, y=Inf_Trab / iteraciones[1] , color=Protocolo)) +
        geom_line(size=1) +
