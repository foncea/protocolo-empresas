...
class AlgoritmoBiosPrevencion(AlgoritmoBios):
    def asignar_estado_segun_test(self):
        if self.resultado_test:
            self.individuo.cambiar_estado_observado('infeccioso')
            self.prevencion = True
            self.tiempo_prevencion = 0
        else:
            self.individuo.cambiar_estado_observado('susceptible')
    ...
    def tick(self):
        self.numero_tests = 0
        if self.prevencion:
            self.tiempo_prevencion += 1 
        else:
            self.tiempo_prevencion = 0
        if  self.tiempo_prevencion > 13:
            self.prevencion = False 
