...
colnames(seir_sim) = c('Days',  'Compartment', 'cantidad')
seir_sim$Compartment[seir_sim$Compartment == 'susceptible'] = 'Susceptible'
seir_sim$Compartment[seir_sim$Compartment == 'expuesto'] = 'Exposed'
seir_sim$Compartment[seir_sim$Compartment == 'infeccioso'] = 'Infected'
seir_sim$Compartment[seir_sim$Compartment == 'recuperado'] = 'Recovered'

p_sim = seir_sim %>%
    ggplot(aes(x=Days, y=cantidad, color=Compartment )) +
        geom_line() + 
        xlab('Days') + 
        ylab('') +
...
seir$Type = 'SEIR Model'
seir_sim$Type = 'Simulation'
contrast = rbind(seir, seir_sim) %>%
    ggplot(aes(x=Days, y=cantidad, color=type)) +
        geom_line() +
        ylab('') +
...
ggsave(paste('contrast r0=', r0, ' pi=', pi, 'iter=', iter, ' ', '02-05.pdf', sep=''), contrast)
...
# Poblacion trajando
{
    ...
    a = trabajando %>%
        ...
        mutate(Trabajando = percent(Trabajando, digits =0), 
               Infecciosos_Totales = percent(Total_Inf / pob[1], digits=0),
               CV = percent(CV, digits=1),
               SD = round(STD, 1)) %>%
